...
import { Engine2 } from "../core/engine2.ts";
import { Simulation } from "../core/sim2.ts";
import type { World } from "../core/sim2.ts";
...
async function renderScene(engine: Engine2, instruction: string): Promise<string> {
	let text = "";
...
async function cmdStart(game: string, runId: string): Promise<void> {
	...
	const engine = await Engine2.create(def, { ...engineOptsFromEnv(), sim, sessionManager });
	try {
...
async function cmdState(runId: string, gameHint?: string): Promise<void> {
	...
	if (!dir) throw new Error(`run "${runId}" 不存在，请先 start`);
	const meta = loadMeta(dir);
	const sim = Simulation.fromWorld(caveSim2, loadState(dir));
...
async function main() {
  ...
  loop2 start [--run <id>]
  loop2 act <意图文本> [--select <选中文本>] [--run <id>]
  loop2 render [--instruction <指令>] [--run <id>]
  loop2 state [--run <id>]
  loop2 wait <n> [--run <id>]
  loop2 reset [--run <id>]
