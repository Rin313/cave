...
function parseArchiveLine(raw: string): ArchiveLine | null {
	...
	return { kind: "record", record: deepFreeze({ seq: seq as number, time: r.time, utterance: r.utterance as string, steps: r.steps as Commit[], ...(narration !== undefined && { narration }) }) };
}

/** 档案全读：缺席即 null；坏行、末尾半行、回合及其表达一并返回（narrations 是 records 的派生视图），装载与诊断共用同一读法。 */
export function readRecords(path: string): { records: ChronicleEntry[]; narrations: { seq: number; narration: string }[]; broken: number; incomplete: boolean } | null {
	...
	for (const raw of text.split("\n")) {
		...
		if (line.kind === "record") {
			...
			if (line.record.narration !== undefined) narrations.push({ seq: line.record.seq, narration: line.record.narration });
		} else broken += 1;
	}
