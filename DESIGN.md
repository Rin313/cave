...
- **持久化**：唯一证据是回合记录，近况是其纯函数投影（作者选择×引擎投影），只在装载与回合边界整体重算，进程重启由记录按序重放重建（不重裁决、不掷骰、逐变更 prev 校验，终态过 admit）。档案是单一追加日志，条目只有回合（证据，每回合恰一，定稿写点）。装载遇序位不接续或变更不可应用即截断至断点前的完好前缀并告警显形，会话文件不动；投影失败是呈现缺陷，在消费时降级，不算装载损坏。
