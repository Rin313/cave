...
- **不变式可见提交**：`InvariantCtx` 携带 `genesis`、`changes`（本次提交的全部变更，含 spawn/despawn）与 `src`。**状态不变式**只读世界——守恒类，防总量漂移；**过渡不变式**读提交——provenance 类，防错误再分配；每条规则/系统的提交独立过墙，过渡不变式的粒度即单次提交。出处是提交级事实（一次提交恒一产出方），只住在提交边界（InvariantCtx.src）——事件流与变更记录是纯内容，步记录不携带出处；法则归因的显影处是 probe 的裁决地图，机器诊断走 Denial.debug。类型契约管词汇，过渡不变式管语义（这一次提交是否必要）。
...
- 直达提案通道：意志的提案必经映射译码，引擎不设第二条提案入口与仪器摇钟——时间控制属于玩家动词与工具层夹具（研究摇钟为合成的内部动词，过同一裁决边界与硬墙）
