...
- **持久化**：唯一证据是回合记录，近况是其纯函数投影（作者选择×引擎投影），只在装载与回合边界整体重算，进程重启由记录按序重放重建（不重裁决、不掷骰、逐变更 prev 校验，终态过 admit）。档案是单一追加日志（`records.jsonl`），每回合恰一行（回合与其表达同条目）：回合于 act 裁决窗口关闭时已完备，定稿写点＝表达落定，二者一次追加——进程在窗口与表达之间崩溃即最后一回合整体回退，已落前缀不被触碰（末尾半行由装载截断、续写前重写修复）；装载不消费表达、不参与重放。装载遇序位不接续或变更不可应用即截断至断点前的完好前缀并告警显形（末尾无换行的半行同此）：截断以重写落地——续写前档案重写为存活回合前缀（各回合的表达随其记录保留），旧全文移存 `records.jsonl.orphan`，不再进入装载；投影失败是呈现缺陷，在消费时降级，不算装载损坏。
