...
**以确定性状态机承担玩法重量、以 AI 承担映射与表达的交互式文字游戏引擎。**
- **文本即操作面**：用自由文本表达操作意图，不限制输入方式（选中场景散文片段 / 输入框 / 快捷键 / 菜单），不限制交互UI。
- **AI 不产生系统后果。** 操作的重量由确定性状态机提供，AI 只负责把结果表达为文学散文。
...
flowchart TD
    ...
    I["意图（自由文本）"] --> M["AI 映射<br/>纯语义解析 · one-shot act 门闩<br/>构造性拒绝 = 空提案"]
...
- **名字是引擎自有词汇**：id 与 name 是每个实体的必备卡（非空字符串，integrity 钉住），随 spawn/despawn 的 C 进出状态，离场名由 despawn 记录承载。指称与键控的锚是 id 与裁决时读态：协议指称恒为 id，规则键控恒读当前态，名字只承担呈现。
...
- **感知是注入的投影**：感知与呈现钩子（g / edgePerception / propPerception / digestExtra / summarize）住 def 不住账本，core 只留机器消费的空槽；缺省全见是零破坏缺省（选择——钩子缺席时取最少销毁信息的形状）。推论：**裁决侧不读投影**——规则分支于世界真相（可达性等机制谓词是投影的原料，不是感知槽；违反现实规则的世界由 authored 形态承载）。（公理一只排除感知谓词作隐藏状态；钩子签名统一收意志锚是选择——单意志下即 def.playerId，参数是多意志变体的泛化关节）
...
- **裁决驱动落钟**：「无裁决即无流逝」是触发源的选择—— AI 的秒级延迟使实时成为另一种体验形态；提案对其自身结果不可见；近况与审计以回合为自然单位。刻数账目：`ticks(s) = ok(s) ? (J.ticks ?? cost(v)) : cost(v)`——时价的定义域是尝试，不是裁决结果。拒绝分支恒为 `cost(v)`，与 J 无关：门/法则/墙/崩溃四面同价，法则不可改写；授予分支的时长是法则的 authored 输出（缺省 `cost(v)`，可上下改写，与世界腔理由同信任级的披露）。**授予与执行分离**：verdict 的 ticks 是授予的权威记录，执行与授予的一致由同一落钟循环按构造成立；授予刻数在裁决入口校验形状（非负整数，违约走不变式通道）。apply = 裁决 → 提交 → 按授予刻数逐刻落钟。刻是世界的因不是意志的果（每刻 systems 照常运行），不是回合的别名；事件流按构造保序，at 坐标使其可核验可复原。
- **拒绝恒价推导**：deny 无效果集，产物 = 裁决位一比特，负半区查询可重复、无效果。若 ticks 依赖 s，则 deny 查询零价零刻：计价单位从尝试漂移到效果，纯信息查询逃出计价域，且测量通道是不可纪律化的钟（理由是 def 文本，纪律可及；钟是协议机械）——`Δt = 0 ⇔ 否`，钟即免费谓词 oracle。恒价下负半区查询价 = `cost(v)` 且必驱动落钟：测量与行动同价，穷举必过世界。`cost(v) = 0`（缺省）属作者定价，反穷举由提案有界（门闩）、指称域成员性统一文案（门）、理由世界腔（法则）承载；更便宜的知识走感知通道或拆动词，不走分支定价。

### 词汇面

- **世界语言**：玩家可见的一切文字（法则理由、不变式 message、回退摘要）与表达契约（voice）；表达散文由AI在契约下生成，对已锚定内容的转写与渲染一律自由。
- **通道语言**：引擎↔prompt 脚手架、act 工具描述与校验反馈、结果视图的协议锚（未解析行、新见 id 卡）——协议基础设施，不进玩家视野；映射契约（one-shot 门闩、空提案、预计被拒也提交、叙述跟随结果）的文案单源，系统块/工具描述/回合提示/门闩四方消费同一措辞。
- **开口键按字面**：谓词的声音面只设在声明卡上——卡随词汇闭合而存在（prop/verb 的 label）；开口 token 层（关系类型、tag token）无卡，机械行按字面言说（键即词），其世界腔由表达层承担。

### 地籍

- **持久化事实 = 地籍条目 = 回合定稿**（intent + steps 原样）。绕过引擎的裸 `apply` 不产生条目——情节记忆（近况）是回合制域，后果的可说性由状态视图独立承载（公理一在状态侧满足）。
- **近况**：Π(条目窗口, w_now)——纯函数投影、不持久化、随时可重建。
- **入账完备性**：刻的唯一驱动者是落钟循环，刻只发生在某次授予的执行内 ⇒ 凡刻必属某步的授予区间，凡步必入回合定稿。
- **名字闭合**：Π 解析的指称只有尝试行 ref 参数与 id 型属性值（理由与 Fact 是内联字面，不过解析）。ref 参数过可见性门 ⇒ 裁决时在世；id 值过 integrity ⇒ 提交时在世；记录在案的变更都不引用已死实体（引用后于死亡的 delta 被墙回滚）⇒ 失效至多发生在引用之后。若引用条目在窗口内，则 despawn 条目不早于它 ⇒ 也在窗口内（条目按时间序、窗口取后缀）⇒ 名字恒可解析：在世取现值（改名连续），离场取 despawn 底表。例外是被拒尝试对幻觉 id 的原样回显——不解析即非合法指称，非闭合破口。
- **玩家原话**：intent（意志当回合的完整言说）逐字入账（近况记录 + 会话审计），投影侧以结构惰性形态渲染；作为后果时经 speech-act 动词物化为账本惰性记录。
...
| `cost` | 尝试时价（刻，缺省 0）：拒绝分支恒价（法则不可改写），`ticks` 只改写授予分支 |
| `ref`／`free` | 字符串参数的 kind 标记（schema 上的声明）。ref＝指称参数，过可见性门：`adm(a, w⁻) ⟺ ∀p ∈ ref(a): a.params[p] ∈ g(w⁻)`（参照域 = `g(w,锚) ∩ E`，谎报的 id 不进视图、不可指名）——判据是参照域成员性，故对一切域外 id 回答恒同（幻觉 id 与隐藏实体同一文案：门对 `E ∖ g` 零泄漏）；机械渲染解析为名字，probe 按其枚举。free＝自由字符串，值按字面进入裁决，径由法则裁决。参照域即「意志能点名什么」——**视野内引用**（实体卡）、**已知引用**（出口、对话获名等：visited/known 标记由规则经 delta 写入，internal 令标记对AI不可见，grounding 并入参照域）。 |
...
- **投影钩子统一收意志锚**（grounding / edgePerception / propPerception / digestExtra / summarize 均收 def.playerId）：钩收锚不收体验者——锚比推导值信息更全，锚的自见排除、按锚键控的知识都需锚本身，体验者相对的投影由此可表达而锚信息不丢；钩子可无视锚（上帝视角即无视锚的常函数）。
- **无主语动词的缺省主语是体验者**——体验者由包含拓扑推导，视角与门随链跟随。第一人称（感知以锚的现宿主为缺省）、上帝视角（感知全开 + 主语显式入参）、附身（占据 = 锚实体 `in` 的迁移，一条 delta 过门；会死会换角色的游戏把锚指到魂/叙述者等不朽实体，占据建模为 id 属性而非关系边——宿主之死才被 id 引用契约拦成显式迁移）都是主体性概念的取值模式。
...
- **键控与载荷**：键控形态下规则必须以属性组合为键，禁止特判实体 id；量级与效果参数存于属性载荷——键控管「何时对谁」，载荷管「多少」，规则随新实体自动泛化（组合爆炸的唯一出路）。
...
- **拒绝**：结构化 Denial（law/reason/debug），理由以世界腔内联在规则文本里，缺省回落 `messages.noResponse`。拒绝不携带涉及实体：指称的落点是 action 参数（结构化）与法则理由（世界腔），第三方指称可由 law 在裁决时世界态上重算。`deniedBy: "rule"`（卫语句链、可见性门、全部未表态的引擎闭合 `action.unanswered`）是玩法——世界性理由；`"invariant"`（硬墙否决、授予形状违约、法则代码失灵 `*.crash`）是必要性拦截——probe 报 bug。可见性门归 rule（拒绝恒价：时价的定义域是尝试，非本分类的推论）。覆盖面没有机械判据：probe 穷举可见域产出裁决地图供人判读兜底落点，缺口判断属创作者。
- **前置门两站点**：静态形态（动词存在、schema）错在裁决之外——工具边界由 pi 校验拒绝，内核收到同类动作即调用方违约，抛 `ProtocolViolation`；世界真相在裁决之内——可见性门（拒绝为世界性、耗尝试时价；未入域的指称在尝试行原样回显、不解析名字）→ 规则。施动前提等语义前提一律写成第一条卫语句规则，谓词由 games 层构件/模块助手供给。**接线判据**：前提的否定情形落在参照域内（可指名而不可及）才接线法则，落在域外（不可指名——暗箱内容物）由门独任，重复接线即死法则；grounding 以可达性谓词定义感知域是合法取值，代价是物理前提被门吸收——grounding 与前提法则是同一次决策的两面，改其一必重审另一；法则的存活条件是世界×认识论的合取，不是接线习惯。probe 的法则×动词活性矩阵（弃权/未达计数、零表态）是键控泛化拓扑的唯一观察处。
...
- **执行翼**：delta 逐条校验而非提交前预检：目标实体与关系端点必须存在、关系类型须为非空字符串（边的身份组成）、spawn 的 id 必须未占用、rename 的名字须为非空字符串、一切数值后果必须有限（NaN/Infinity 不合法）；一切账本值必须可单行线性化。幂等跳过的唯一判据是后态已成立：`skip(δ) ⟺ cur = post(δ)`（relSet：`prev = v`；set：`cur(k) = v`，以目标存在为前提）。裁决理由与变更流必须互证。
- **状态翼**：默认恒挂 integrity——id 唯一、卡片契约（id 与名字非空）、钟契约（time 非负整数）、id 型属性指向存在的实体（无引用由缺席表达）、账本形状封闭（世界/实体/关系边顶层键闭合、存储值非 null、标量或标量数组——边值与属性值同一契约；any 只豁免标量类型检查、不豁免账本值形状）、边身份契约（类型为非空字符串 token、三元组唯一）、注册表类型契约与词汇闭合（实体属性键 ⊆ 注册表：type 层封闭、token 层开放，与动词表同构；自由值形状走 any 与 tags，动态键值对走关系边）；游戏追加领域不变式；守恒类种子读 `InvariantCtx.genesis`（实际起点世界的冻结快照，存档恢复与变体开局同样成立）。

- **不变式可见提交**：`InvariantCtx` 携带 `genesis`、`changes`（本次提交的全部变更，含 spawn/despawn）与 `src`。**状态不变式**只读世界——守恒类，防总量漂移；**过渡不变式**读提交——provenance 类，防错误再分配；每条规则/系统的提交独立过墙。事件流与变更记录是纯内容，步记录不携带出处；法则归因的观察处是 probe 的裁决地图，机器诊断走 Denial.debug。
- **`invariant` 是独立的否决来源**：法则否决是玩法（世界性理由），墙否决是必要性拦截，审计与探测应能区分。
- **指针不可重绑**——居所引用在 def 层恒定；附身/换躯不改指针（占据是锚实体 `in` 属性的迁移）。**锚实体不朽，宿主可生灭**——墙保护的是锚的指向：终局是状态不是删除；占据建模为 id 属性时，宿主之死被 id 引用契约拦下——迁出必先作为 delta 入账（迁移可说，静默回退不可说），迁移合法性由过渡不变式钉住；推导兜底（hostOf 缺省锚自身）只覆盖「未入链」态，不豁免迁移的可说性。

### 生灭与拓扑

- Δ 五原语：set 值 null 即删键、relSet 值 null 即删边；null 是缺席的记号，不入存储，缺席的唯一形态是键不在场，记忆由变更记录承担（prev/next: null）。提交产出按基底类别同构分形的变更记录 C：rel 携带完整边端点（next null 即删边；despawn 级联删边同一形状）、rename 携带 prev/next 名字、spawn 携带完整后态实体、despawn 携带展示名——离场名的唯一来源。变更序列是后态的完整 diff：`w_t = w₀ ⊕ C₁ ⊕ … ⊕ Cₙ`，逐步合成即重放任意时刻的世界。
...
- **状态视图**：`E_v = { card(e) : id(e) ∈ g(w) }`，`card(e).props = { k ∈ keys(e) : k ∉ internal ∧ π_p(e,k) }`；`R_v = { r ∈ R : r.from, r.to ∈ g(w), π_e(r) }`——视图实体索引 ≡ 可见性门权威集（AI看得见的才可指名），卡上属性包同理（可感的值必在卡上）。游戏派生纹理 `digestExtra` 入独立 `extra` 键（形态自由 ViewValue）——无引用声明面：纹理以名字陈述可指名者（实体卡已在视图），以字面模糊披露不可指名者（行动须先物化为已知引用）。act 结果的**新见段**是同一装配线的回合内增量：本回合新进参照域的实体以状态视图同形的实体卡（含 id）承载——纹理与指称锚在裁决当回合闭合。
...
- **属性感知**：`propPerception`（同形空槽：世界 × 体验者 → 槽谓词，收实体×键）声明体验者知觉哪些属性值，缺省恒真；同一谓词约束状态视图的 props 块与事件投影的 prop 变更行，快照随步携带。截面 = 实体×注册表键的元组编码，枚举注册表而非在场键——缺席槽可感（清空后的「→ null」仍可说）。谓词读物化真相：「知其有不知其值」的知识居所仍是账本，感知不物化为内容。槽知觉与宿主知觉正交：prop 行的指称集只含值（占据迁移行系于宿主指称，不系于锚自身可见性），宿主可见性不是截面的维度。
- **事件投影**：act 结果视图与回退摘要是世界对体验者的应答，与状态视图消费同一个 grounding；行可说性按 speak 判据（见世界代数），渲染不扩权。名字解析与可说性分立：ref 参数与 id 型属性值解析为名字——域内取现值，窗口内离场者取 despawn 底表；域外引用原样回显（活体名解析仅限已感知指称，否则尝试行成为隐藏实体的存在性 oracle）；未声明为引用的字符串一律字面，与实体 id 的碰撞不铸造指称。离场底表是纯名字面（离场名曾随实体卡公开，不因离场方式失密）；隐藏离场者自身的行恒沉默（span 不含离场者），跨度可说的历史行离场后仍以离场名渲染；沉默行的净效果由状态投影承接（新见/离场），模糊知觉的唯一通道是作者 Fact。理由与 Fact 不过投影——规则铸造的世界语，作者特权（summarize 声音覆写）是唯一合法越权口。span 由 Simulation 产出步时计算（动作步前态复用可见性门，刻步按系统提交逐个取），随步携带——与 Change.prev/next 同一本体：提交后不可重算、只存在于记录（变更自含、骰子纯函数使重放重算技术上可行，但重算即用提交后的知识改写历史——记录须是证据而非视图）。AI所得 = Π(S_now) ⊕ 被感知事件 ⊕ 作者 Fact。
...
- **纯语义解析**：无关键词/正则匹配、无别名表、无语言限制。
- **one-shot 门闩**：AI先调 `act` 一次性提交动作提案（或以空提案拒绝），门闩封的是本回合的第二次调用。
...
- **回合骨架（spineLines）**：事件流的规范单行渲染——符号承担结构（✓/✗/⏱/×n）、语言词全部来自 messages/label/规则文案，internal 变更恒滤。结果视图、近况、控制台与缺省回退摘要共用同一线级机械；回退摘要缺省 = 骨架投影；`summarize` 是可选的声音覆写——读原样 steps（与理由/Fact 同属作者特权），崩溃回落缺省，呈现缺陷不得丢弃已发生的账目。
...
**方法＝覆盖矩阵**：机制维度×通道粒度求覆盖，空格即候选；每个候选再过四条证伪路线——**换通道**（边→属性、拒绝→零 delta 授予、提问→拒绝理由）、**物化**（历史/预期/信念→属性）、**拆动词**（成本不对称、条件 schema→动词分裂）、**键设计**（命运共享、效果叠加→地址/数据归约）。四路皆不通者才是「无法表达」候选，且仍须由真实探针实例化方可立项（准入原则不变）。
...
- 数据法则（AST解释器）：无明显收益
- 回合纪要：自述偏差经上下文跨回合放大
- 直达提案通道：意志的提案必经映射译码，不设第二条提案入口与仪器摇钟——时间控制属于玩家动词与工具层夹具
