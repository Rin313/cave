...
- 无 label 且非 identity 的属性（内部变量）与被 `present: "hidden"` 遮蔽的注册边不进状态视图、事件投影与近况；`private` 动词不进动词面（动词面 = 动词表 ∖ private）；private ⇒ clock（def 期校验）：private 即不广告的常驻声音，由泵逐刻调用，`private ∧ ¬clock` 不可达。
...
骰子地址 `addr = (at, origin, verb, 序位)`——入账表态的账本位置；序位 = 同 (at, origin, verb) 的已入账表态数（本步之前）。地址对入账表态单射且由账本前缀唯一确定：`origin` 与 `action` 随步入账，序位按入账表态计数。默（时钟的全弃权与空授予）没有账本位置、不消耗序位；其若调用 roll，取「若入账则应有」的位置，同由账本前缀与自身 (at, origin, verb) 决定。不入 `params`：自由文本参数入地址会让玩家输入左右随机；同地址恒同值，sides 违约即抛。
...
**回合定稿**　`(seq, time, utterance, steps)` 入账；绕过引擎的裸 apply 不产生条目、不入账本——引擎实例的变异恰经回合协议，否则世界不可由账本重建。定稿单点原子且前移到窗口关闭：回合的内容于裁决完成时已完备，叙述不在定义内，act 工具尾落条目即定稿——表达中断只降级呈现（回落确定性摘要），账目照记。落盘是定稿的完成条件：条目未落地即回合未入账。
...
**呈现服务**　narrate 无提案通道、无 act 通道、无时间流逝，不是回合；输出不入账、不进投影。用途：开场与场景重渲。

## 面向作者的契约

- **动词表**：`params` 声明（type×可选×重数×描述）派生接口模式、内核校验与规则参数的编译期类型；`type ∈ {lit(string), lit(number), lit(boolean), ref}`，`ref` 的值是实体 id（过指称门＝所见集），`lit(·)` 是字面；`many` 令参数为非空序列，与世界值同形——一次尝试的操作数是裁决的一部分（指称逐项过门、整次原子），批次是多个尝试在世界态上的顺序 fold；操作数与顺序组合是两根轴，多重性不由批次承载。`private` 动词不进动词面（动词面 = 动词表 ∖ private，以文本广告与接口模式两种编码呈现）：private ⇒ clock（def 期校验），private 即不广告的常驻声音，由泵逐刻调用。`clock` 动词每刻由泵以空参提案过同一扇门：无价、全弃权即默、授予禁携刻；其 deny 发声为失败刻。origin 由调用点决定：will（玩家经动词面）、clock（泵）；private 只管动词面（private ⇒ clock，故不经 will）。origin 随步入账，投影不再回查 def。
