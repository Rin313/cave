...
**视角是注入的投影**　规则只读世界真相；访问结构不进规则，只经引擎的门进入提交（`refers` 即门域）。可达性等机制谓词用于投影；呈现期读 `sees`、`refers` 与 `name`；命名不是第二套披露判定，门也不经命名。
...
内核不持行动主体：视角是 `World → Access` 的纯函数，主体是游戏的法则约定（参数字段或从内容推导），二者都不进账本形状。
