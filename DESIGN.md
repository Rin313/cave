...
- 无 label 且非 identity 的属性（内部变量）与被 `present: "hidden"` 遮蔽的注册边不进状态视图、事件投影与近况；`private` 动词不进动词面（动词面 = 动词表 ∖ private；仍可由代码直连 apply；其步不产尝试行，后果由状态视图与新见段承接）。
...
voice 是一次尝试的答复句（授予与否决皆可，至多一句）；facts 是授予在变更之外仍要说出的零至多句世界陈述；否决没有 facts（世界未变，没有后果可陈述）。二者的差别只在规范渲染中的位置。
...
骰子地址 `addr = (at, origin, verb, 序位)`——入账表态的账本位置；序位 = 同 (at, origin, verb) 的已入账表态数（本步之前）。地址对入账表态单射且由账本前缀唯一确定：`origin` 与 `action` 随步入账，序位按入账表态计数。未入账的尝试（默）没有账本位置、不消耗序位；其若调用 roll，取「若入账则应有」的位置，同由账本前缀与自身 (at, origin, verb) 决定。不入 `params`：自由文本参数入地址会让玩家输入左右随机；同地址恒同值，sides 违约即抛。

### 步

```
Commit ::= (at, origin, action, price, 果)
origin ∈ { will, clock, code }        -- 来源：调用点决定，只分流行为
action ::= (verb, params)             -- 公共载荷；clock 的 params 恒空
price  =  origin = clock ? 0          -- 时钟步不计价
...
序列按构造保序，at 为钟坐标。origin 由调用点决定（will：玩家经动词面；clock：泵逐刻；code：引擎直连 apply），随步入账——记录自含分类与价格，投影不重跑裁决、不据动词表反推（label/messages 等呈现词汇仍取自 def）：一切步一律携 action（action.verb 即动词；clock 的 params 恒空，是空参提案，不是另一种形状）。`果` 是裁决点——授予记法则，否决记 `Point`（法则/门/闭合/审查/自检/崩溃）与受众；提交存在判据：变更 ∨ 事实 ∨ 否决 ∨ 应答义务；尝试恒因应答义务入账，时钟的空授予（无变更、无事实）即默、不留空步。步入账即冻结（构造后不可变）。呈现按 origin 分流：will 产尝试行（✓/✗ 动词与理由），code 不入事件流（不产尝试行，后果由状态视图与新见段承接），clock 归 ⏱（授予成块、否决为失败刻、全弃权即默）；时钟授予携刻 → `engine(grant)` 否决。
...
**内容与坐标**　`E, R` 是内容，门的原子域；`t` 是坐标，不产 `𝒞`，写者唯一（落钟循环），谓词照常读钟。钟的每次写入都归因于某次授予的落钟（刻账目）；回滚的坐标边界由账目裁定：已入账的刻不可回滚，未入账的刻必须一并回滚。
...
**随机是账本位置的纯函数**　同地址恒同值 ⇒ 事件重放与存档恢复得到同一结果。
...
**事件投影**　AI 所得 = `Π(窗口, def) ⊕ 感知变更行 ⊕ 作者 voice/facts`。近况 `Π` 是记录的纯函数且不持久化（记录的提交边界由账本末世界逆推重建，脸表与披露谓词即时求值，投影不回读活世界）：窗口内回合记录按与 act 结果相同的变更行判据投影（推论·刻账目闭合），体量由 `recentWindow` 独自调节；言默同源重算。act 结果的新见段是同一投影的回合内增量：本回合新进所见域的实体以状态视图同形的实体卡承载。

## 主体性

**锚**　`def.playerId` 是指向普通实体的引用；意志不在世界里。
...
**通道次序**　整批先过静态形态检查再占窗口，随后批次执行是 apply 的 fold——后一动作在后一世界态上裁决。静态违约不产生步骤：边裁边检会让中途违约在步流中留下缺口；混批任一违约则整批不进裁决。
...
**回合定稿**　`(seq, time, utterance, steps)` 入账；绕过引擎的裸 apply 不产生条目。定稿单点原子且前移到窗口关闭：回合的内容于裁决完成时已完备，叙述不在定义内，act 工具尾落条目即定稿——表达中断只降级呈现（回落确定性摘要），账目照记。落盘是定稿的完成条件：条目未落地即回合未入账。
...
- **动词表**：`params` 声明（type×可选×重数×描述）派生接口模式、内核校验与规则参数的编译期类型；`type ∈ {lit(string), lit(number), lit(boolean), ref}`，`ref` 的值是实体 id（过指称门＝所见集），`lit(·)` 是字面；`many` 令参数为非空序列，与世界值同形——一次尝试的操作数是裁决的一部分（指称逐项过门、整次原子），批次是多个尝试在世界态上的顺序 fold；操作数与顺序组合是两根轴，多重性不由批次承载。`private` 动词不进动词面（动词面 = 动词表 ∖ private，以文本广告与接口模式两种编码呈现），由代码直接 apply——同一裁决边界与审查；其步因 origin=code 不产尝试行，后果由状态视图与新见段承接。`clock` 动词每刻由泵以空参提案过同一扇门：无价、全弃权即默、授予禁携刻；其 deny 发声为失败刻。origin 由调用点决定：will（玩家经动词面）、clock（泵）、code（引擎直连）；private 只管动词面，与 origin 无关——公开动词亦可由 code 触发（不产尝试行），价对 code 通道照常计 cost。origin 随步入账，投影不再回查 def。
