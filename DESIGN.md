...
```
World  ::= (E, Rel)          -- 世界不含钟：E, Rel 是内容，时间只是账本的读数
...
- 无名的格不进状态视图、事件投影与近况（缺省实现下即 `label` 缺席或为 `null` 的属性、`label: null` 的注册边）；常驻规则（`GameDef.ticks?`）不进动词面、不携参数与跨度，由泵逐拍以空参 Q 过同一扇门。
- 名字可随知识（视角）变化，故「不认得的名字」「无名者」是 token 的取值；改名不是格——一步内顶点 token 变化即派生出 `~ 旧 → 新`，两边界皆在 H 内才有名可换。
...
```
G : Δ* × ⟨rule⟩ → 𝒞 ⊎ Denial                原子：拒绝 ⇒ 不产出 𝒞（E、Rel 不变）
...
- 卫语句链 `rules`（动词与常驻规则同构）：链上每条守卫携链内唯一 id；首个非 ⊥ 表态即判决，授予可携 `law`（缺省即守卫 id），否决的 `law` 由 `deny` 给定——记录两侧自含 (守卫, law)。全弃权由引擎闭合为 `closure`。指称参数先过指称门（域即可指称集），非指称参数按字面径由法则裁决（引擎不解析）。拍内授予不得携 span（不得延伸跨度）——引擎点否决。
...
- 骰子地址 `addr = (act 序号, trigger, verb, 拍位)`——尝试在账本中的位置；act 序号 = 该尝试所属 act 的序号（0 起，act 步即自身，clock 步即其跨度所属的 act），拍位 = act 步 0、clock 步为其跨度内第几拍。地址对尝试单射且由账本前缀与负载唯一确定：跨度不入地址，等待不左右随机（拍位标定同一跨度内的各拍；同拍多规则共享拍位，凭 verb 区分）。入账与否不改变地址（弃权与入账同址同值）；地址不入 params：自由文本入地址会让输入左右随机；同地址恒同值；sides 违约即抛。
...
- 入账内容：凡裁决时读出且不可由账本前缀回算者随步入账。act 步记 span，clock 步记 offset（静默拍不留步；泵按 span 逐拍产生 offset ∈ [1, span] 且同跨度内非降）；边界值 `prev` 一并入账，供渲染与重放消费；提交产出与装载接受共用单步形状判据，跨步一致性不对账（见非目标）。读数不满足入账原则（无人裁决它），故不是世界字段：装载按记录重建世界，读数由账本前缀派生（见「账本与刻度」）。由 kind 唯一决定的呈现身份不入账（lawOf 产生）；只读派生视图（脸表、sees、言默、近况、账本读数）一律是投影。
...
viewBase(w, r) = ( readout = r,               -- r 为账本读数（Σ act span），非世界字段
...
**账本与刻度**　`E, Rel` 是内容，门的原子域。世界不含钟：没有坐标字段，只有账本读数——账本序给出先后，act 步的 `span` 给出度量，clock 步的 `offset` 给出拍位，`readout = Σ act span`（原点 gauge，只供呈现）。泵是调度原语而非时间本体：按 span 逐拍运行常驻规则，不写任何世界字段；裁决不读读数，故读数不满足入账原则。记账律：act 的 span 即其跨度，clock 步不延伸跨度；已入账的跨度不可回滚，未入账的跨度必须一并回滚。方向由「账本只追加 ∧ span ≥ 0」给出——它是账本的结构，不是世界的属性。
...
- Token消耗审计
- 回合数统计
- 日志防篡改与账目对账：装载是重建，不是审计（只校单步形状，跨步 span/offset 一致性不查）；投影失败是消费事件，降级呈现
