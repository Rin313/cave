...
- **忠实性约束（强制，非提示词）**：
  ...
  - **后置校验器**：声明实体必须 ⊆ 可见实体，且命中**本回合状态可推导集**——动作主体（玩家）、本回合新可见实体（如打开容器后露出的物品）、变更（from/to）、法则 facts、「即将发生」、**授予动作的实体参数**（被拒动作参数不在内——被拒动作未改变任何状态）。散文正文不再有机械检查（`leakageCheck` 已移除——与 `forbiddenTerms` 同类：词表式匹配 + 语言假设，交由 prompt 设计与模型合规）；**新事实一律走声明头**，`text` 不得断言状态中不存在的实体/属性、不得与状态矛盾——状态蕴含的旧事实（陈设、属性、位置）允许描述。
