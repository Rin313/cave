...
core 的承诺分三层：**公理**是定义性承诺；**协议不变量**是公理的直接推论，对一切协议变体强制；**回合协议**是本引擎在不变量之上选定的缺省协议实例。**指不出推导来源或选择理由的引擎形态即设计警报**。
...
- **可说性**：账本的唯一职责是让真相可说、可 diff：凡产生后果者，其后果必可说——变更说为 C、氛围与理由说为 F、时间流逝说为步记录里经裁决归因的刻数；中间态经 `internal` 标记合法不可说。
...
- **一回合一意志**：act 是言语行为通道，每回合恰好一个说话人。意志即提案者，提案通道两个合法形态：经映射译码（act 工具，产品形态）与直达受理（不经译码——研究仪器/作者代码；直达回合无门闩，门闩封的是模型不是提案者）。意志不在世界里——账本里没有玩家，`def.playerId` 是指向普通实体的**居所引用**（integrity 墙管辖、指针不可重绑）。多意志不触任何公理与不变量，是未实例化的协议变体，若实例化：每意志一个居所引用、提交者入步记录——单意志下该义务恒退化为省略。
...
- **持久化事实 = 地籍条目 = 回合定稿**（intent + steps 原样，写点唯一 = 回合定稿）。直达回合的 intent 是提案者（操作者/作者代码）的声明，逐字入账；绕过引擎的裸 `apply` 不产生条目——情节记忆（近况）是回合制域，后果的可说性由状态视图独立承载（公理一在状态侧满足）。
...
- **玩家原话**：intent 逐字入近况 + 会话审计；作为后果时经 speech-act 动词物化为账本惰性记录。
...
- **核心增长准入：只有「无法表达」（而非「表达痛苦」）有资格驱动 core 的机制增长。**
...
| `narratableChanges`／`viewCard` | 变更行／实体卡 | 全局 internal 滤 | 事件／感知 |
...
**方法＝覆盖矩阵**：机制维度（灵感来源表即维度清单）×通道粒度求覆盖，空格即候选；每个候选再过四条证伪路线——**换通道**（边→属性、拒绝→零 delta 授予、提问→拒绝理由）、**物化**（历史/预期/信念→属性，缴物化税）、**拆动词**（成本不对称、条件 schema→动词分裂）、**键设计**（命运共享、效果叠加→地址/数据归约）。四路皆不通者才是「无法表达」候选，且仍须由真实探针实例化方可立项（准入原则不变）。

已判定的存活格（直觉候选均被四路线击杀，例：效果叠加→键设计的数据归约；失忆→地籍是框架不是世界，记忆物化；前瞻→世界是数据，judge 内克隆自演；拒绝浮动成本→零 delta 授予；知其有不知其值→边承载存在＋internal 属性承载值＋digestExtra 重露）：

1. **属性隐藏的粒度不一致**：`internal` 是唯一无体验者维度的隐藏通道，违背「感知与呈现钩子统一携带体验者参数」的工程约定。状态面经 digestExtra 重露可闭合；**机械事件面不可闭合**（`narratableChanges` 全局滤、digestExtra 无 steps 参数）——per-experiencer 秘密的 prev→next 机械行对任何人不存在，事件叙述只能由作者 Fact 代笔（双写税）。这不是某游戏建不出，是通道代数与声明的对称性相抵触。
2. **批次内真同时性**：applyBatch 是 fold，单意志下休眠，多意志变体实例化时激活。

**可证伪预测**：单意志游戏中，状态/动力学/感知面几乎完全闭合；未来 core 增长只可能来自上述两格。侵蚀度量由此获得具名格子与可检验预测，而非「希望预测为零」。
...
覆盖不了的残余，显式声明：**多意志同时性**是协议变体（引擎层），任何 GameDef 都不覆盖；**批次多动作压力**与 **e2e 会话规模**属于 loop 阅读与夹具，不属于游戏层。

统一探针＝`seals`，展品清单（展品→格子）：

| 展品 | 格子 |
|---|---|
| 信文 internal＋digestExtra 重露＋法则代笔 | 存活格 1：状态面闭合／机械事件面不可说（税） |
