...
- **表达层声明契约（唯一契约 structured，工具化）**：新事实经第二个自定义工具 `declare` 以结构化参数提交（每条 `{ entities: string[], statement }`），涉及集推导与校验集中在 `core/declare.ts`（单一来源：`validateFactIds`）。校验为集合成员判断——实体须可见且属本回合**涉及集**（touched：actor + 法则 facts + 新见 + 变更/即将发生 + 授予动作参数；被拒动作参数不在内——被拒动作未改变任何状态），无名字回退。逐条错误即时返回，模型在同一 turn 内自我纠正（无需重试 prompt）。**Fact 认识论（§4-12）**：percept（缺省）facts 的实体进 touched；utterance（世界之言，如梦中低语）不进——话语只许被转述，不许据以断言状态。
...
- **`playerId`（意志锚点，def 指向世界的唯一数据引用）**：主体性在 core 只有两个概念——**意志**（玩家）与**视角**（感知函数的现值，DESIGN §3.1）；本字段钉死前者：act 通道意图的**归属**实体（意图属于玩家，实体是它在世界的住址），无主语动词的缺省主语、声明契约 touched 集的常任成员、第二人称叙述的受话人。视角与身体不在其命名范围：视角是游戏注入的感知函数的现值——**引擎缺省全见全达即上帝视角，第一人称是游戏声明（village/yume 注入 grounding/reach/holdable），不是引擎立场**；身体（现宿主）是游戏从世界态推导的取值（附身时名义宿主退为空壳），三者汇聚于同一实体是自我中心游戏的声明模式，不是焊点。实证一（playerId 洞，最小 def + despawn 主体的动词）：despawn(player) 被授予后，解引用 `q.actor` 的规则从 `apply` 直接抛 TypeError，系统对悬空身体返回 ok=true 零变更（静默假成功，同 §4-5 类）；两门闭案：integrity 不变式恒查 `ids.has(def.playerId)`（过渡门），Simulation 构造器对初始世界跑全部不变式（genesis = 自身、changes = 空）（初始门——公理 6「不变式管永远」下 t=0 原是必要性自由区；def 结构错误与损坏存档在装载时即响）。实证二（视角实验，零 core 改动）：上帝视角以「缺省感知 + 主语显式入参的动词（feed(eater, store)）+ 神力动词（spawn）」成立，后果落在被指令者、两扇门照常；附身以「钩子从世界态推导现宿主 + 规则推导施事」成立——可见性门自动跟随视角（意志锚点不动，器皿动）。由此声明的立场：**指针不可重绑**（意志锚点在 def 层恒定）；**名义宿主不朽，器皿可生灭**——墙保护的是意志的住址而非任何身体：终局是状态不是删除（先例：village `down`、yume `ended`），附身的器皿可 despawn，意志按游戏推导回退到名义宿主。引擎真正焊死的只有一件事：每回合恰好一个意志（act 是言语行为通道，一次吟唱一个说话人；多意志即协议变更 + 回合调度，属另一品类）。游戏侧的 `props.actor: true`（systems 迭代的复数标记）与 `kind: "actor"` 是游戏词汇，与意志锚点的一致性属游戏自律（core 不内嵌属性名，不可执法）。
