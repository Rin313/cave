...
5. **回合编排（单 pass 部署）** 每回合一次 `session.prompt()`：模型先调 `act`（一次性提交，one-shot 门闩封闭变异窗口），工具结果即本回合世界回应的世界腔策展（尝试行/时间流逝/法则事实，经 fmtChange 线性化；新见以状态视图同形的实体卡承载视图增量），模型基于它输出散文；叙述只能跟随工具结果。
