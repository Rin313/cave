...
10. **structured-only 声明契约取代 prose + 词表断言扫描器——e2e 净胜，确定性复现"子串匹配不可维护"的最终结局。** 对比实验（waste/village × baseline(prose + assertionRules) / structured(唯一契约、无扫描器) × 每组独立开局 × 基础 + 压力意图集，全样本 200+ act）：
