...
- **通用断言校验（声明式）**：`GameDef.assertionRules`（`{ prop, weak?, strong?, targets, exemptPending?, impossibleOnly?, error }`）+ `negationWords` / `sentencePunct` / `assertionPunct`（词表/标点由游戏注入，引擎不感知语言）。engine 在泄漏检查后自动执行 `checkAssertions`（core 通用算法）：散文断言了与物理状态相反的事实即报错；**`impossibleOnly` 持有类断言的文学宽松开关**——core 以 `input.wieldable`（grabbable+reach）收窄 targets，只拦"不可能持有"（沉重或够不着）的声称，可达可持握物品的"随手拿起/放下"视为动作叙述（waste 的 `in` 规则已启用，village 保持严格口径）；weak 断言词豁免"即将发生"（pending）的预言实体，strong 断言词不豁免（如「烧成灰烬」不能因下一 tick 就豁免），`exemptPending: false` 的规则（如「持有」）不接受预言豁免。
- **表达层声明契约（`declarationContract`，GameDef 可选，缺省 "prose"）**：`[facts: ...]` 声明的校验方式由游戏选择，拆分/泄漏检查/涉及集推导集中在 `core/declare.ts`（单一来源）。prose 契约：事实为自然语言，名字子串 + 最长命中归属 + 不可见实体检查（`validateDeclAInv`）；structured 契约：事实写作 `[id1,id2]: 陈述`，按实体 id 精确集合校验、无名字回退（`validateDeclB` strict；解析器括号组安全，支持内嵌方括号的多实体写法）。
